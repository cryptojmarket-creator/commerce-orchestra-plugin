...
Entitlement checks send an installation identifier, request identifier, requested action, and plugin version to the entitlement service. The response contains access status, plan, remaining allowance, renewal timing, and a short-lived lease. Account creation and any payment take place through a separately operated website; the plugin does not receive payment details.
