...
Subscription checks send an installation identifier, requested action, and plugin version to the entitlement service. The response contains access status, plan, remaining allowance, renewal timing, and a short-lived lease. Billing credentials are not returned to Codex.
